...
function NotificationModel({ visibility, functions }) {
    ...
    useEffect(() => {
        ...
        if (bottomObserverElement.current && watchLaterData?.length) {
    ...
    }, [watchLaterData, handleObservers]);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (parseData && parseData?.length > 0 && !isAllDataLoad) {
                    ...
                    const { status, data, dataIsEnd } = await loadMoreFetch({
                        ...
                        apiPath: `${appConfig.backendUrl}/api/v1/user/watch_later`,
                        bodyData: {
                            saveData: parseData
                        },
                        limitPerPage: 5,
                        skip: watchLaterData.length,
                        page
                    });

                    if (status === 200) {

                        setWatchLaterData((prev) => [...prev, ...data.moviesData]);
