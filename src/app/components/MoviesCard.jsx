        ...
        <>
            {moviesData?.map((data) => (
                <div key={data._id} onClick={()=> navigatePage(data.watchLink)} className="bg-yellow-800 cursor-pointer relative movies_image_container">
                    ...
                    <div className="absolute top-1 left-1 z-10 w-auto h-auto px-2.5 py-1 bg-purple-700 text-white text-xs text-center font-sans font-semibold rounded-md">{data.releaseYear}</div>
                </div>
            ))}
        </>
    )
