                ...
                <div className="w-auto h-auto flex items-center gap-2 px-2 py-3 justify-around border-b border-gray-300">
                    {notifactionTypesArray.map((type, index) => (
                        <button key={index} onClick={() => setSelectedNotifactionType(type)} role="button"
                            className={`transition-all duration-200 ease-in-out text-xs border rounded-xl px-3 py-1 capitalize ${selectedNotifactionType == type ? "bg-blue-50 text-blue-600 border-blue-400 font-semibold" : "bg-gray-100 text-gray-900 border-gray-500"}`}>
                            {type}
                        </button>
                    ))}
                    ...
                    {filteredNotifactions?.length > 0 ? (
                        <>
                            {filteredNotifactions?.map((data) => (
                                <div key={data.id} className="w-auto h-auto flex gap-2 items-center px-2.5 py-2 border-b border-gray-300 hover:bg-gray-100">
                                    <i className="bi bi-chat-left-text-fill text-gray-500"></i>
