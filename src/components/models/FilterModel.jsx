...
const FilterModel = memo(({ initialFilterData, filterData, functions, filterOptions }) => {
    ...
    const [selectedFilter, setSelectedFilter] = useState(initialFilterData);
    ...
    useEffect(() => {
        if (!isInitialFilterChange) {
            setSelectedFilter(initialFilterData);
        } else {
    ...
    const sortFilterOptions = [
        { filterLabel: 'New to old', sort: { dateSort: -1 } },
        { filterLabel: 'Old to new', sort: { dateSort: 1 } },
        { filterLabel: 'Recent added', sort: { dateSort: 'recent added' } },
        { filterLabel: 'Rating high to low', sort: { ratingSort: -1 } },
    ];
    ...
    const addFilter = (newFilterData) => {
        ...
        if (isFilterSelected) {
            ...
            setSelectedFilter(updatedFilterData);
        } else {
            setSelectedFilter({ ...selectedFilter, ...newFilterData });
        }
    };
        ...
        <>
            ...
            <div className={`w-auto h-auto bg-white fixed bottom-1.5 right-0 transition-all duration-500 ease-in-out z-20 ${visible ? 'translate-y-0' : '-translate-y-[-110%]'} border border-gray-300 shadow-2xl py-1 rounded-md select-none`}>
                ...
                <div className="w-56 h-auto pb-2">
                    {sortFilterOptions.map((data, index) => {
                                ...
                                <summary
                                    ...
                                    <span className="shrink-0 transition duration-300 group-open:-rotate-180">
                                        <svg
                                            ...
                                            <path
                                                fillRule="evenodd"
                                                d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 1.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 010-1.414z"
                                                clipRule="evenodd"
                                            />
                                        </svg>
                                    </span>
                    ...
                    <div className="w-auto h-auto border-t border-gray-200 mt-2 pt-1">
                        <button onClick={applyFilter} disabled={!isApplyFilter} className={`w-full h-auto px-3 py-1 text-white text-xs font-medium ${isApplyFilter ? "bg-cyan-500 hover:bg-cyan-600" : "bg-gray-200 cursor-not-allowed"} rounded transition-all duration-500 ease-in-out`}>Apply</button>
                        <button onClick={clearFilter} disabled={!isClearFilter} className={`w-full h-auto px-3 py-1 mt-1 text-gray-700 text-xs font-medium ${isClearFilter ? "bg-gray-200 hover:bg-gray-300" : "bg-gray-50 cursor-not-allowed"} rounded transition-all duration-500 ease-in-out`}>Clear</button>
                    </div>
                </div>
            </div>
        </>
